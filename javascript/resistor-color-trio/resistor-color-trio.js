...
export class ResistorColorTrio {
  ...
  get label() {
    if (!this.colors.every((color) => resistorColors.includes(color))) {
      throw new Error(/invalid color/);
    }
    let value =`${resistorColors.indexOf(this.colors[0])}${resistorColors.indexOf(this.colors[1])}`;
    let unit = "ohms";
    for (let i = resistorColors.indexOf(this.colors[2]); i !== 0; i--) {
      value += 0;
    }
    if (value > 1000) {
      unit = "kiloohms";
