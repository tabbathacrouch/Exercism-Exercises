...
export class GradeSchool {
  ...
  add(name, grade) {
    let currentStudents = this._roster[grade] || [];
    Object.values(this._roster).filter(x => x.indexOf(name) !== -1).forEach(x => x.splice(x.indexOf(name),1));
    currentStudents.push(name);
    this._roster[grade] = currentStudents.sort();
  }
